...
export default function CreatePropertyPage() {
  ...
  useEffect(() => {
    const savedDraft = localStorage.getItem('rw-property-create-draft');
    if (savedDraft) {
      try {
        setFormData(JSON.parse(savedDraft));
        setAutosaveMsg('Restored draft from local storage');
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      localStorage.setItem('rw-property-create-draft', JSON.stringify(formData));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Format request payload to match backend DTO
    const payload = {
    ...
    try {
      ...
      setSuccess(true);
      localStorage.removeItem('rw-property-create-draft'); // Clear autosave draft on success
      setTimeout(() => {
        navigate('/listings');
      }, 2000);
    } catch (err: any) {
      console.error(err);
      setError(err.response?.data?.message || 'Validation failed. Please check your fields.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleClearDraft = () => {
    if (window.confirm('Clear all form fields and delete saved draft?')) {
      setFormData(initialFormState);
      localStorage.removeItem('rw-property-create-draft');
      setAutosaveMsg('Draft cleared');
      setTimeout(() => setAutosaveMsg(null), 2000);
    }
  };
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8 animate-fade-in">
      <div className="mb-6 flex items-center justify-between">
        ...
        {autosaveMsg && (
          <span className="text-xs text-indigo-400 flex items-center gap-1.5 px-3 py-1 rounded-full bg-indigo-500/10 border border-indigo-500/20">
      ...
      <div className="glass border border-slate-800 rounded-3xl p-6 md:p-8">
        <div className="flex justify-between items-start border-b border-slate-900 pb-6 mb-8">
          <div>
            ...
            <p className="text-slate-400 text-sm mt-1">
              Provide property descriptions, prices, location, and metadata. Saved instantly to drafts.
            </p>
          </div>
        ...
        {success ? (
          <div className="text-center py-12">
            ...
            <h2 className="text-2xl font-bold text-slate-200">Listing Draft Created!</h2>
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            {/* Section 6: Enriched Optional Metadata */}
            <div className="space-y-4">
              <h2 className="text-lg font-bold text-indigo-400 flex items-center gap-2 border-b border-slate-900 pb-2">
            ...
            {/* Actions Footer */}
            <div className="pt-6 border-t border-slate-900 flex justify-end items-center gap-4">
              <button
                type="button"
                onClick={() => navigate('/listings')}
